...
def plot_signal_with_slopes_and_r2(x, y, valid_peaks, window=10):
    ...
    ax.plot(x, y, label="Original CH₄ signal", color="blue", alpha=0.6)
    ...
    for i, (start, end) in enumerate(segments):
        ...
        if r_value**2 > 0.7:
            ...
            plt.text(mid_x, mid_y, f"Slope: {slope:.2f}\n$r^2$: {r_value**2:.2f}",
                    fontsize=13, color='darkred', ha='center', bbox=dict(facecolor='white', edgecolor='none', alpha=0.7))

    ax.set_xlabel("Time (s)", fontsize=22)
    ax.set_ylabel("CH₄ (ppm)", fontsize=22)
    ax.set_title("Slopes & $r^2$ of Signal Segments", fontsize=24)
...
def process_file (filepath, output_dir, window_peaks=5):
    ...
    time = df["time(s)"].values # Normalize time to start from 0
    ...
    # Comparison plot of original and Processed CH₄ signals with detected peaks
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(time, signal, label="Original CH₄ signal", color="gray", alpha=0.5)
    ax.plot(time, final_signal, label="Processed CH₄ signal", color="blue", linewidth=1)
    ax.scatter(time[valid_peaks], signal[valid_peaks], color='red', marker='o', label="Validated Peaks", zorder=5)

    ax.set_ylabel("CH₄ (ppm)", fontsize=22)
    ax.set_title("Original vs Processed Signals with Peaks", fontsize=24)
    ax.tick_params(axis='both', labelsize=16)
    ax.legend(fontsize=16)
    ax.grid(True)
    ...
    flux_lines, flux_series = calculate_slopes_and_difusive_flux(
        ...
        temperatures_C=df['Temp'] if 'Temp' in df.columns else df['tempC'],
        pressures_mmHg=760,
        volume_m3=0.011,
        area_m2=0.0782,
        window=10,
    ...
    ax.set_ylabel("CH₄ (ppm)", fontsize=22)
    ax.set_title("Analog-like response of valid peaks", fontsize=24)
    ax.tick_params(axis='both', labelsize=16)
    ax.grid(True)
    ax.legend(fontsize=16)
    fig.tight_layout()
